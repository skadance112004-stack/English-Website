...
interface GenerateSpeakingRequest {
  exerciseId:    string;
  userPrompt:    string;
  documentText?: string;
  exerciseMeta: {
  ...
  lineCount:       number;   // how many lines to generate (4-10)
  currentLines:    { speaker: string; preview: string }[];
}

interface GenerateSpeakingResult {
  reasoning:      string;
...
function checkRateLimit(uid: string): void {
  ...
  if (!e || now > e.resetAt) { rateLimitMap.set(uid, { count: 1, resetAt: now + 60_000 }); return; }
  if (e.count >= 10) throw new HttpsError("resource-exhausted", "Too many requests. Please wait.");
  e.count++;
}
...
async function verifyTeacher(uid: string): Promise<void> {
  ...
  if (snap.data()?.role !== "teacher") throw new HttpsError("permission-denied", "Only teachers can use AI generation.");
}

async function checkTokenBudget(uid: string): Promise<void> {
  const today = new Date().toISOString().slice(0, 10);
  const snap  = await admin.firestore().collection("users").doc(uid).collection("ai_usage").doc(today).get();
  if ((snap.data()?.tokensUsed ?? 0) >= 50_000) throw new HttpsError("resource-exhausted", "Daily AI token limit reached.");
}

async function recordTokenUsage(uid: string, tokens: number): Promise<void> {
  const today = new Date().toISOString().slice(0, 10);
...
async function writeAILog(uid: string, targetId: string, prompt: string, generated: object, tokens: number): Promise<string> {
...
function parseLines(raw: string): AILine[] {
  ...
  try {
    ...
    return parsed.map((l: any) => ({
      ...
      vocabularyHelp:     Array.isArray(l.vocabularyHelp)
        ? l.vocabularyHelp.map((v: any) => ({ word: String(v.word || ""), definition: String(v.definition || "") }))
        : [],
...
export const generateSpeakingContent = onCall(
  ...
  async (request): Promise<GenerateSpeakingResult> => {
    ...
    const {
      exerciseId, userPrompt, documentText, exerciseMeta,
      lineCount = 6, currentLines = [],
    } = request.data as GenerateSpeakingRequest;
